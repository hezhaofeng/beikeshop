...
- 待支付催款（仅在后台订单详情页手动发送，可按支付方式配置专用模板）
...
邮件发送沿用系统邮件引擎和队列设置；订单类自动邮件在事务提交后发送，避免订单回滚产生误发。“订单待支付”和“待支付催款”都先按订单的 `payment_method_code` 精确匹配支付方式专用模板，未配置或未匹配时使用各自的通用模板；新增支付插件启用后会自动出现在配置页。待支付催款不会自动发送，配置并启用后，在后台订单详情页点击“发送催款邮件”手动发送。插件保留核心注册、订单、退货邮件，不会替换或禁用核心通知。
