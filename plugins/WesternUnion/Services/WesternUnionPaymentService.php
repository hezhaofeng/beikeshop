...
class WesternUnionPaymentService extends PaymentService
{
    public const CODE = 'western_union';
    ...
    /**
     * 初始化服务时限制订单必须使用Western Union，避免其他支付方式复用人工核验流程。
     */
    public function __construct($order)
    {
        ...
        if ($this->paymentMethodCode !== self::CODE) {
            throw new \LogicException('订单支付方式不是Western Union。');
        }
    }
    ...
    public function instruction(array $setting = []): array
    {
        ...
        if ($transferInstruction === '') {
            throw ValidationException::withMessages([
                'transfer_instruction' => trans('WesternUnion::common.transfer_instruction_not_configured'),
            ]);
        }
    ...
    public function verifiedPayment(array $data): array
    {
        ...
        if (! hash_equals($expectedAmount, $receivedAmount)) {
            throw ValidationException::withMessages([
                'received_amount' => trans('WesternUnion::common.amount_mismatch', [
    ...
    public static function formatAmount($amount): string
    {
        if (! is_numeric($amount)) {
            throw ValidationException::withMessages([
                'received_amount' => trans('WesternUnion::common.invalid_amount'),
            ]);
        }
    ...
    /**
     * 读取并限制Western Union折扣百分比，避免配置值影响其他支付方式。
     */
    public static function discountPercentage(array $setting): float
    {
        // 新插件默认沿用原线下转账插件的 8% 优惠；显式填写 0 仍可关闭。
        $percentage = $setting[self::DISCOUNT_PERCENTAGE] ?? 8;
    ...
    /**
     * 判断当前购物车是否超过后台配置的Western Union件数门槛。
     */
    public static function shouldForceWesternUnionPayment(array $setting, int $quantity): bool
    {
