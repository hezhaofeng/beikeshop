...
namespace Plugin\WesternUnion\Controllers;
...
class WesternUnionController
{
    ...
    {
        return view('WesternUnion::shop.order_lookup');
    }
    ...
    public function declare(Request $request, string $number): JsonResponse
    {
        ...
        $this->assertPendingWesternUnionOrder($order);
        ...
        try {
            DB::transaction(function () use ($order, $data, $receiptPath, &$previousReceipt): void {
                ...
                $this->assertPendingWesternUnionOrder($lockedOrder);
                $previousReceipt = $lockedOrder->orderPayments()->value('receipt');

                $service = new WesternUnionPaymentService($lockedOrder);
        ...
        return json_success(trans('WesternUnion::common.declaration_saved'));
    }
    ...
    public function confirm(Request $request, Order $order): RedirectResponse
    {
        ...
        return redirect(admin_route('orders.show', $order))->with('success', trans('WesternUnion::common.payment_verified'));
    }
    ...
    private function paymentVerifiedComment(Order $order): string
    {
        ...
        try {
            return trans('WesternUnion::common.payment_verified');
        } finally {
    ...
    {
        $this->assertWesternUnionOrder($order);
    ...
    private function validateDeclaration(array $data, bool $hasExistingReceipt = false): void
    {
        ...
        $receiptRequired = WesternUnionPaymentService::requiresReceipt($setting['receipt_required'] ?? true);
        ...
        if ($receiptRequired && ! $hasReceipt && ! $hasExistingReceipt) {
            throw ValidationException::withMessages([
                'receipt' => trans('WesternUnion::common.receipt_required_error'),
            ]);
        }

        if (! $hasReceipt && ! $hasExistingReceipt && $transactionId === '') {
            throw ValidationException::withMessages([
                'transaction_id' => trans('WesternUnion::common.declaration_evidence_required'),
            ]);
        }
    }
    ...
    private function storeReceipt(Request $request, Order $order): ?string
    {
        ...
        return $file->storeAs('western-union-receipts/' . $order->id, $fileName, 'local');
    }

    /**
     * 人工声明和到账确认都只接收仍待支付的Western Union订单。
     */
    private function assertPendingWesternUnionOrder(Order $order): void
    {
        $this->assertWesternUnionOrder($order);

        if ($order->status !== StateMachineService::UNPAID) {
            throw ValidationException::withMessages([
                'order' => trans('WesternUnion::common.order_not_confirmable'),
            ]);
        }
    }

    /**
     * 私有凭证只能关联Western Union订单，防止跨支付方式读取文件。
     */
    private function assertWesternUnionOrder(Order $order): void
    {
    ...
    {
        return is_string($path) && preg_match('#^western-union-receipts/\d+/[a-f0-9-]+\.(?:jpg|jpeg|png|gif|webp|pdf)$#', $path) === 1;
    }
}
